...
data_train=data_train.reshape(-1,28*28)[1:1000,:]/(28*28)
targets_train=targets_train[1:1000]
data_test=data_test.reshape(-1,28*28)[1:1000,:]/(28*28)
targets_test=targets_test[1:1000]
...
sample_sizes = 300 * np.arange(10, 15)
fourier_scores = []
nystroem_scores = []
fourier_times = []
nystroem_times = []
...
for D in features:
    ...
    fourier_score = fourier_approx_svm.score(train_X, train_y)
    fourier_scores.append(fourier_score)
plt.plot(features,fourier_scores)
plt.xlabel('n° RFF')
plt.ylabel('Accuracy')
plt.grid()
plt.show()
